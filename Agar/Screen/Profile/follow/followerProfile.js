...
import
 {
    ...
    const ref = firebase.database().ref('FollowProfile/'+`${user}/`+'Follower')
   
  
    

export default followerProfile = ({navigation}) => {
  ...
  function getFollowerProfile(){
    ...
    followerList.map((a)=>{
          return(
            db.ref('kullaniciBilgiler/'+a.followerUser)
            .once('value',snapshot=>{
              const data = snapshot.val();
              const list = Object.values(data)
              setUserFollowerList(list)
