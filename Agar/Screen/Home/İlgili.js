...
export default İlgili = ({navigation,route}) => {
...
const [followCategoryName, setFollowCategoryName] = useState([]);
...
//BURASI KULLANICININ BEĞENDİĞİ KONULARDAN OLUŞAN KISIM OLACAK
...
function getComments(){
  ...
  ref.once("value",snapshot=>{
    ...
    snapshot.forEach(child => {
      child.val().category.map(follow=>{
        followCategoryName.map((item)=>{

        if(follow.name == item.name){
            li.push({
              id:child.val().id,
...
function getFollowCategory(){
  const user = firebase.auth().currentUser.uid;
  const db = firebase.database();
  const ref = db.ref('Begeniler/'+`${user}`);
  ...
  ref.once('value',snapshot=>{
    ...
    snapshot.forEach(child=>{
      ...
      const fCategory = Object.values(data)
      setFollowCategoryName(fCategory)
...
useEffect(() => {
  ...
  },[]); //buraya followcategoryname eklenecek fakat ondan önce 
  //storage kısmında ki url kullanıcı profiline aktarılacak oradan çekilecek
